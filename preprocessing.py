...
def load_data_monti(dataset, testing=False, rating_map=None, post_rating_map=None):
    ...
    # O_group_test: (290, 7710), O_group_train: (290, 7710)

    M_user = load_matlab_file(path_dataset, 'M_user')
    M_group = load_matlab_file(path_dataset, 'M_group')
    M_group_user = load_matlab_file(path_dataset, 'M_group_user')

    O_group_train = load_matlab_file(path_dataset, 'O_group_train')
    O_group_test = load_matlab_file(path_dataset, 'O_group_test')

    num_users = M_user.shape[0]
    num_groups = M_group.shape[0]
    num_items = M_user.shape[1]

    assert(M_user.shape[1] == M_group.shape[1])
    assert(M_user.shape[0] == M_group_user.shape[1])
    assert(M_group.shape[0] == M_group_user.shape[0])
